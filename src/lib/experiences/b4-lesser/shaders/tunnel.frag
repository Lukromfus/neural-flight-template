...
void main() {
  ...
  // 3. EXTREME Normal Mapping
  vec3 n = normalize(vNormal + normalMap * 2.5); 
  vec3 viewDir = normalize(cameraPosition - vWorldPosition);
  vec3 lightDir = normalize(vec3(0.0, 0.0, -1.0)); 
  vec3 halfVec = normalize(lightDir + viewDir);

  // 4. SEAMLESS Heatmap Hue Generation
  float angle = vUv.y * 3.14159 * 2.0;
  vec3 seamlessPos = vec3(cos(angle) * 2.0, sin(angle) * 2.0, vUv.x * 10.0);
  float heatNoise = fbm(seamlessPos * 0.5 + vec3(0.0, 0.0, uTime * 0.4));
  ...
  // Creates sweeping volumetric bands of deep shadow and intense brightness through the tunnel
  float lightWave = sin(vWorldPosition.z * 0.04 + uTime * 2.5) * 0.5 + 0.5;
  finalColor *= mix(0.15, 1.85, lightWave); 
