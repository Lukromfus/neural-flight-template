...
class B5LesserEffect extends Effect {
  ...
  setCA(v: number): void {
    (this.uniforms.get("uCA") as THREE.Uniform<number>).value = v;
  }
  setWarp(v: number): void {
    (this.uniforms.get("uWarp") as THREE.Uniform<number>).value = v;
  }
  setVig(v: number): void {
    (this.uniforms.get("uVig") as THREE.Uniform<number>).value = v;
  }
  setFlicker(v: number): void {
    (this.uniforms.get("uFlicker") as THREE.Uniform<number>).value = v;
  }
  setHeartbeat(v: number): void {
    (this.uniforms.get("uHeartbeat") as THREE.Uniform<number>).value = v;
  }
  setTime(v: number): void {
    (this.uniforms.get("uTime") as THREE.Uniform<number>).value = v;
  }
}

function createCylGeo(
  r: number,
  len: number,
  segs: number,
  rings: number,
...
function createStarField(
  ...
  rMax: number,
  lenZ: number,
  colorFn: (r: number) => [number, number, number],
  ...
  for (let i = 0; i < count; i++) {
    ...
    pos[i * 3 + 2] = -50 + Math.random() * (lenZ + 100);
    const c = colorFn(Math.random());
    col[i * 3] = c[0];
    col[i * 3 + 1] = c[1];
    col[i * 3 + 2] = c[2];
    sz[i] = szRange[0] + Math.random() * szRange[1];
...
class TrailObject {
  ...
  private readonly _pp: Float32Array;
  private readonly _pc: Float32Array;
  private readonly _ps: Float32Array;
  private readonly _fp: Float32Array;
  private readonly _fc: Float32Array;
  private readonly _fs: Float32Array;
  ...
  constructor(i: number) {
    ...
    this.dot = new THREE.Mesh(
      ...
      new THREE.ShaderMaterial({
        ...
        uniforms: {
          uCol: { value: new THREE.Color(0.95, 0.92, 0.88) },
          uPulse: { value: 1.0 },
        },
        transparent: true,
        depthWrite: false,
  ...
  pos(t: number, camZ: number, leviathanMerge: number = 0): THREE.Vector3 {
    ...
    const normalPos = new THREE.Vector3(
      Math.cos(a) * r,
      Math.sin(a * 1.23) * r * 0.75,
      z,
    );

    if (leviathanMerge > 0) {
      const maw = new THREE.Vector3(
        Math.sin(t * 5 + this.ph) * 0.5,
        -4,
        camZ + 35,
      );
  ...
  update(
    ...
    for (let i = 0; i < HIST; i++) {
      ...
      if (i < n) {
        ...
        this._pp[j + 2] = h.z;
        const al = i / n;
        this._pc[j] = 0.9 + al * 0.1;
        this._pc[j + 1] = 0.4 * al;
        this._pc[j + 2] = 0.05 * al;
        this._ps[i] = (0.38 + al * 4.5) * sc;
      } else {
        this._pp[j] = this._pp[(n - 1) * 3];
        this._pp[j + 1] = this._pp[(n - 1) * 3 + 1];
        this._pp[j + 2] = this._pp[(n - 1) * 3 + 2];
        this._ps[i] = 0;
      }
    }
  ...
  hide(): void {
    ...
    this.fPts.visible = false;
  }
  addToScene(scene: THREE.Scene): void {
    scene.add(this.dot, this.pPts, this.fPts);
  }
  dispose(): void {
    this.dot.geometry.dispose();
    (this.dot.material as THREE.Material).dispose();
    this.pPts.geometry.dispose();
    (this.pPts.material as THREE.Material).dispose();
    this.fPts.geometry.dispose();
    (this.fPts.material as THREE.Material).dispose();
  }
}
...
interface CageUniforms {
  uTime: THREE.Uniform<number>;
  uInten: THREE.Uniform<number>;
  uCol: THREE.Uniform<THREE.Color>;
}
...
export interface B5LesserState extends ExperienceState {
  ...
  tunnelU: TunnelUniforms;
  tunnelLight: THREE.PointLight;
  tunnelHalo: THREE.Mesh;
  tunnelHaloU: TunnelHaloUniforms;
  ...
  tunnelParticleMat: THREE.ShaderMaterial;
  tunnelFog: THREE.Mesh[];
  tunnelFogMat: THREE.ShaderMaterial[];
  canalMesh: THREE.Mesh;
  canalMat: THREE.ShaderMaterial;
  canalScrollOffset: number;

  circleSymbol: THREE.Mesh;
  triangleSymbol: THREE.Mesh;
  circleGlowU: GlowUniforms;
  triangleGlowU: GlowUniforms;
  ...
  trailSc: number;
  gpMesh: THREE.Points;
  gpGeo: THREE.BufferGeometry;
  gpSamp: THREE.Vector3[];
  threats: THREE.Mesh[];
  threatGlowU: GlowUniforms[];
  _onResize: () => void;
...
const CAGE_LEN = 400;
const GP_N = 850;
const CANAL_RADIUS = 3.5;
const CANAL_LENGTH = 700;
const CANAL_NEAR = 10;
const CANAL_FAR = 10 - CANAL_LENGTH;
const CANAL_RAD_SEG = 32;
const CANAL_Z_SEG = 100;
...
export async function setup(ctx: SetupContext): Promise<B5LesserState> {
  ...
  const tunnelLight = new THREE.PointLight(0x4a1a88, 3.2, 28);
  scene.add(tunnelLight);
  const tunnelAmbient = new THREE.AmbientLight(0x120a2a, 0.2);
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const idx = Math.min(
      tunnelFrames.normals.length - 1,
      Math.floor(t * tunnelFrames.normals.length),
    );
  ...
  const fogF =
    "uniform float uTime; uniform vec3 uColor; varying vec2 vUv; void main(){ float d = distance(vUv, vec2(0.5)); float a = smoothstep(0.5, 0.0, d) * 0.07; a *= 0.5 + 0.5 * sin(uTime + vUv.x * 3.0); gl_FragColor = vec4(uColor, a); }";
  ...
  for (let i = 0; i < 90; i++) {
    ...
    const fogMat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uColor: { value: new THREE.Color(0.5, 0.25, 0.85) },
      },
      transparent: true,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
    ...
    fog.position.set(
      path.x + Math.cos(ang) * rad,
      path.y + Math.sin(ang) * rad,
      zPos,
    );
  ...
  const circleGlowU: GlowUniforms = {
    uCol: new THREE.Uniform(new THREE.Color(0.9, 0.75, 0.2)),
  ...
  circleSymbol.layers.set(1);
  circleSymbol.visible = false;
  scene.add(circleSymbol);
  ...
  const triangleGlowU: GlowUniforms = {
    uCol: new THREE.Uniform(new THREE.Color(0.3, 0.7, 1.0)),
    uPulse: new THREE.Uniform(1.0),
  };
  ...
  triangleSymbol.layers.set(2);
  triangleSymbol.visible = false;
  scene.add(triangleSymbol);
  ...
  const cageU: CageUniforms = {
    uTime: new THREE.Uniform(0.0),
    uInten: new THREE.Uniform(0.0),
    uCol: new THREE.Uniform(new THREE.Color(0x1a3acc)),
  };
  ...
  scene.add(stars);
  const nebula = createStarField(
    900,
    8,
    CAGE_R - 1,
    CAGE_LEN,
    (r) => (r < 0.5 ? [0.06, 0.12, 0.48] : [0.22, 0.04, 0.42]),
    [0.9, 3.5],
  );
  scene.add(nebula);

  const trails: TrailObject[] = Array.from(
    { length: TRAIL_N },
    (_, i) => new TrailObject(i),
  );
  ...
  for (let i = 0; i <= 400; i++) {
    ...
    gpRel.push(
      new THREE.Vector3(
        Math.sin(t * 6.8 + 0.4) * 2.6 +
          Math.cos(t * 14 + 1.1) * 1.1 +
          Math.sin(t * 3.2) * 1.8,
        Math.cos(t * 5.3 + 0.8) * 2.0 +
          Math.sin(t * 9.2 + 0.5) * 1.2 +
          Math.cos(t * 2.1) * 1.4,
        z,
      ),
    );
  ...
  const gpPA = new Float32Array(GP_N * 3),
    gpCA = new Float32Array(GP_N * 3),
    gpSA = new Float32Array(GP_N);
  for (let i = 0; i < GP_N; i++) {
    gpCA[i * 3] = 1.0;
    gpCA[i * 3 + 1] = 0.82;
    gpCA[i * 3 + 2] = 0.18;
    gpSA[i] = 0.35 + Math.random() * 0.4;
  }
  const gpGeo = new THREE.BufferGeometry();
  gpGeo.setAttribute("position", new THREE.Float32BufferAttribute(gpPA, 3));
  gpGeo.setAttribute("aCol", new THREE.Float32BufferAttribute(gpCA, 3));
  gpGeo.setAttribute("aSz", new THREE.Float32BufferAttribute(gpSA, 1));
  const gpMesh = new THREE.Points(
    gpGeo,
    new THREE.ShaderMaterial({
      vertexShader: particleVert,
      fragmentShader: particleFrag,
  ...
  gpMesh.visible = false;
  scene.add(gpMesh);

  const threatGlowU: GlowUniforms[] = [];
  const threats: THREE.Mesh[] = Array.from({ length: 8 }, (_, i) => {
    const gu: GlowUniforms = {
      uCol: new THREE.Uniform(new THREE.Color(0xcc6600)),
      uPulse: new THREE.Uniform(1.0),
    };
    threatGlowU.push(gu);
    const m = new THREE.Mesh(
      new THREE.SphereGeometry(0.7 + Math.random() * 0.45, 14, 10),
      new THREE.ShaderMaterial({
        vertexShader: frVert,
        fragmentShader: glowFrag,
        uniforms: gu as any,
  ...
  const startScreen = new THREE.Sprite(
...
export function tick(
  ...
  if (s.phase === 0) {
    ...
    tCA = 0.52 + Math.min(s.phaseT / 30, 1) * 0.48;
    tWarp = Math.min(s.phaseT / 30, 0.75);
  } else if (s.phase === 4) {
    tCA = 1.2;
    tWarp = 0.9 + Math.sin(elapsed * 8) * 0.3;
    tFlick = 0.4;
  } else if (s.phase === 5) {
    tCA = Math.max(0, 0.14 - s.phaseT * 0.01);
    tWarp = 0.0;
    tFlick = 0.0;
  }
  ...
  const hbRaw = Math.pow(
    Math.max(0, Math.sin(elapsed * Math.PI * 1.15 - 0.2)),
    10.0,
  );
  ...
  const hbPhase =
    s.phase === 0
      ? s.hbPulse * 0.3
      : s.phase === 1
        ? s.hbPulse * Math.max(0, 1.2 - s.tunnelReveal * 2.0)
        : s.phase === 5
          ? s.hbPulse * 0.5
          : 0;
  ...
  if (s.phase === 0) {
    s.renderer.toneMappingExposure +=
      (0.5 - s.renderer.toneMappingExposure) * delta * 0.5;
    ...
    const forwardSpd =
      (s.tunnelSpeed * 6 + pitchFactor) *
      (accelerate ? 1.8 : brake ? 0.3 : 1.0);
    ...
    const exitIntensity = Math.min(1, 200 / Math.max(distToEnd, 1));
    s.exitLight.intensity = 2 + exitIntensity * 8;
    (s.exitGlow.material as THREE.MeshBasicMaterial).opacity = Math.max(
      0.1,
      exitIntensity * 0.6,
    );
    ...
    s.renderer.toneMappingExposure +=
      (1.0 - s.renderer.toneMappingExposure) * delta * 0.5;
    ...
    let sysInf =
      s.phase === 3 ? Math.min(1, s.phaseT / 30) : s.phase === 4 ? 1.0 : 0;
    const forcedRoll = Math.sin(elapsed * 0.08) * 15 * sysInf;
    const blendRoll =
      roll * (1 - sysInf * (s.phase === 4 ? 1.0 : 0.8)) + forcedRoll;
    ...
    const wPull =
      s.phase >= 3 ? s.postWarp * Math.sin(elapsed * 0.55) * 3.8 : 0;
    s.camera.position.set(s.camPos.x + bobX, s.camPos.y + bobY, s.camPos.z);
    s.camera.lookAt(
      s.camPos.x + Math.sin(s.heading) * 22 + wPull,
      s.camPos.y + blendPitch * 9 + bobY * 2,
      s.camPos.z + Math.cos(s.heading) * 22,
    );
    ...
    if (s.phase === 2 && s.phaseT >= s.phaseDuration) {
      s.phase = 3;
      s.phaseT = 0;
    } else if (s.phase === 3 && s.phaseT >= s.phaseDuration) {
      s.phase = 4;
      s.phaseT = 0;
    }
  } else if (s.phase === 5) {
    s.tunnelProgress = Math.min(
      1,
      s.tunnelProgress + (s.tunnelSpeed * delta * 0.8) / TUNNEL_LEN,
    );
    ...
    const fi = Math.min(
      s.tunnelFrames.normals.length - 1,
      Math.floor(t * s.tunnelFrames.normals.length),
    );
    ...
    s.renderer.toneMappingExposure +=
      (0.8 - s.renderer.toneMappingExposure) * delta * 0.3;
  }

  const inCanal = s.phase === 1;
  s.canalMesh.visible = inCanal;
  s.exitGlow.visible = inCanal;
  ...
  if (showCurved) {
    ...
    s.tunnelParticleMat.opacity = Math.min(
      0.6,
      (s.tunnelParticleMat.opacity ?? 0) + delta * 0.4,
    );
    ...
    for (let i = 0; i < s.tunnelFog.length; i++) {
      s.tunnelFogMat[i].uniforms.uTime.value =
        elapsed + (s.tunnelFog[i].userData.offset as number);
    }
  ...
  s.closedDoor.visible = showExit;
  s.openDoor.visible = showExit;
  if (showExit) {
    ...
    if (s.renderer.xr.isPresenting) {
      ...
      if (xrCam.cameras && xrCam.cameras.length === 2) {
        const left = xrCam.cameras[0],
          right = xrCam.cameras[1];
        let layerLeft = showExit && s.dominantEye === "right" ? 2 : 1;
        let layerRight = showExit && s.dominantEye === "left" ? 1 : 2;
        left.layers.enable(0);
        left.layers.enable(layerLeft);
        left.layers.disable(layerRight);
        right.layers.enable(0);
        right.layers.disable(layerLeft);
        right.layers.enable(layerRight);
      }
    } else {
  ...
  s.stars.visible = showCage;
  s.nebula.visible = showCage;
  if (showCage) {
    s.cageMesh.position.set(0, 0, s.camZ - 80);
    s.cageU.uTime.value = elapsed;
    s.stars.position.set(0, 0, s.camZ - 50);
    s.nebula.position.set(0, 0, s.camZ - 50);
  ...
  const trailScTarget =
    s.phase >= 3 ? 1.5 + Math.sin(elapsed * 1.9) * 0.5 : 1.0;
  ...
  if (s.gpMesh.visible) {
    ...
    for (let i = 0; i < GP_N; i++) {
      pa[i * 3] = s.gpSamp[i].x;
      pa[i * 3 + 1] = s.gpSamp[i].y;
      pa[i * 3 + 2] = s.gpSamp[i].z + s.camZ;
    }
...
export function dispose(state: ExperienceState, scene: THREE.Scene): void {
  ...
  s.startScreen.material.dispose();
  (s.startScreen.material as THREE.SpriteMaterial).map?.dispose();
  ...
  scene.remove(s.tunnelLight);
  s.tunnelLight.dispose();
  scene.remove(s.exitLight);
  s.exitLight.dispose();
  s.exitGlow.geometry.dispose();
  (s.exitGlow.material as THREE.Material).dispose();
  scene.remove(s.exitGlow);
  ...
  for (const t of s.trails) {
    t.dispose();
    scene.remove(t.dot, t.pPts, t.fPts);
  }
